...
def extract_scenes(url, model, title, video_id):
    ...
    d = 0
    ret_scenes = {"title": title, "video_id": video_id,
                  "url": url, "highlights": []}
...
def get_category(id):
    # Get category
    urlCat = 'https://www.googleapis.com/youtube/v3/videos?part=snippet&id=' + \
        id + '&key=AIzaSyA2cu1skGcRjDfIpG2I1ri_MWeObrZGS30'
...
def get_videos(channelID):
    url = "https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=50&order=date&type=video&videoDuration=medium&channelId=" + \
        channelID + "&key=AIzaSyA2cu1skGcRjDfIpG2I1ri_MWeObrZGS30"
...
if nn_model == None:
    ...
    classifier = MLPClassifier(activation='logistic', max_iter=1000,
                               learning_rate='adaptive', hidden_layer_sizes=np.full((7, ), 30))
    classifier.fit(X, y)
    nn_model = classifier
...
for title, url in zip(titles, urls):
    if count < 5:
        thread = Thread(target=extract_scenes, args=(
            "https://www.youtube.com/watch?v=" + url, nn_model, title, url))
